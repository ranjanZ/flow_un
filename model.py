...
from keras.layers import Conv2D, BatchNormalization, MaxPooling2D, UpSampling2D, Add, Dropout, concatenate,Flatten,Dense,Lambda,Reshape,LocallyConnected2D,SeparableConv1D,LocallyConnected1D,LeakyReLU
import os
...
def compile_model(model,b_size):
    ...
    loss=K.mean(K.abs(model.inputs[1]-input1_rec))
     


    #loss1=DSSIMObjective(kernel_size=50)(model.inputs[1],input1_rec)
    #loss2=DSSIMObjective(kernel_size=50)(model.inputs[0],input0_rec)
    #loss11=DSSIMObjective(kernel_size=20)(model.inputs[1],input1_rec)
    #loss22=DSSIMObjective(kernel_size=20)(model.inputs[0],input0_rec)
    #loss=loss1+loss2+loss11+loss22
...
def compile_model_new(model,b_size,lambda1=0.02):
    ...
    #re_loss=K.mean(K.abs(model.inputs[1]-input1_rec))
